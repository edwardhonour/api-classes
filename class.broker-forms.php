...
//
	...
	function getCompanyDashboard($data) {
		...
		$sql="select * from nua_doc where employee_id = 0 and company_id = " . $data['id'] . " and doc_title not in ('QUOTING','ENROLLMENT')";
		$p=$this->X->sql($sql);
		$doc=array();
		foreach($p as $q) {
...
//--
	function getInvoiceList($data) {
		...
		if ($output['user']['forced_off']>0) return $output;
		$user=$output['user'];
                $sql="select * from nua_company_invoice order by company_name";
                $d=$this->X->sql($sql);
                $list=array();
                foreach($d as $e) {
                    $sql="select count(*) as c from nua_census where company_id = " . $e['company_id'];
                    $g=$this->X->sql($sql);
                    $e['count']=$g[0]['c']; 
                    array_push($list,$e);
                }
