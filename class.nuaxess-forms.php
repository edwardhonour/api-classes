		 ...
		 //
	 	 ...
	 	 $sql="select * from nua_company_invoice where month_id = '" . $data['id2'] . "' and company_id = " . $data['id'];
		     ...
		     $sql="update nua_company_invoice set invoice_number = '";
                     ...
                     $sql="select * from nua_company_plan where company_id = " . $data['id'] . " order by invoice_order, plan_code";
                     $plans=$this->X->sql($sql);
                     $grand_total=0;
                     $sub_total=0;
		     $adj_total=0;
			  ...
			  $employee_id=$z[0]['employee_id'];
		           ...
		           $employee_id=$this->X->post($p);
                     ...
                     if ($post['medical_plan_code']!="") { 
                           $sql="select * from nua_company_plan where company_id = " . $post['company_id'] . " and plan_code = '" . $post['medical_plan_code'] . "'";
                           $t=$this->X->sql($sql);
                           $plan=$t[0];
...
if ($p['employee_code']=="") {
 ...
 if (sizeof($emps)>0) {
	...
	function getAddEmployee($data) {
		 ...
		 $sql="select * from nua_company_plan where plan_type = '*DENTAL*' and company_id = " . $company_id . " order by plan_code";
		 $p=$this->X->sql($sql);
		 $plans=array();
		 foreach($p as $p2) {
                      $z=array();
		 ...
		 $sql="select * from nua_company_plan where plan_type = '*VISION*' and company_id = " . $company_id . " order by plan_code";
		 $p=$this->X->sql($sql);
		 $plans=array();
		 foreach($p as $p2) {
                      $z=array();
...
// Members loaded in INVOICE LOAD
	...
	function getDuplicateEmployees($data) {
		...
		$census=$census0[0];
                ...
                $sql="select * from nua_company_plan where APA_CODE = '" . $census['apa_plan'] . "' and company_id = " . $census['company_id'];
                $z=$this->X->sql($sql);
		...
		if ($z[0]['c']==0) {
		    ...
		    $sql="select * from nua_company_plan where company_id = " . $cid . " and APA_CODE = '" . $B[0]['plan'] . "'";
		    $C=$this->X->sql($sql);
...
$output['org_name']="Not Found";
		...
		$sql="select * from nua_doc where doc_title not in ('ENROLLMENT','QUOTING') and employee_id = 0 and company_id = " . $data['id'];
		$p=$this->X->sql($sql);
		$doc=array();
		foreach($p as $q) {
			// get the ID as an int.
...
$output['total_revenue']=$total_revenue;
		...
		$output['contribution_levels']=$r;
                ...
                $sql="select * from nua_company_plan where company_id = " . $data['id'] . " order by plan_name";
		$e=$this->X->sql($sql);
                $output['plans']=$e;
	...
	function getEmployeeDashboard($data) {
		...
		if (sizeof($d)>0) {
			...
			$output['formData']=$d[0];
                        ...
                        $sql="select * from nua_monthly_member_census where employee_id = " . $data['id'] . " and month_id = '" . $month_id . "' order by last_name, first_name";
			$j=$this->X->sql($sql);
			$jj=array();
			foreach($j as $k) {
